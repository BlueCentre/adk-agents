...
def create_debugging_agent(name_suffix=""):
    """Factory function to create debugging agent instances.

    Args:
        name_suffix: Optional suffix to append to agent name (e.g., "enhanced_")

    Returns:
        LlmAgent: Configured debugging agent instance
    """
    agent_name = f"{name_suffix}debugging_agent" if name_suffix else "debugging_agent"
