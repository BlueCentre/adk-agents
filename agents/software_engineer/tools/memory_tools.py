...
def add_memory_fact_func(args: dict, tool_context: ToolContext) -> AddMemoryFactOutput:
    ...
    try:
        ...
        error_msg = f"Error adding memory fact: {e!s}"
        logger.error(error_msg)
        return AddMemoryFactOutput(status="error", message=error_msg, key=key, category=category)
    ...
    try:
        ...
        for cat in categories_to_search:
            ...
            for key, value in _memory_store[cat].items():
                ...
                if not query or query.lower() in key.lower() or query.lower() in str(value).lower():
        ...
        error_msg = f"Error searching memory: {e!s}"
        logger.error(error_msg)
        return SearchMemoryFactsOutput(
            status="error", query=query, category=category, results={}, total_categories=0
        )
