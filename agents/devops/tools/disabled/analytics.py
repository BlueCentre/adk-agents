...
class ToolAnalytics:
    ...
    def record_tool_execution(self, tool_name: str, start_time: float, end_time: float,
                            input_size: int, output_size: int, success: bool,
    ...
    def _update_tool_metrics(self, record: ToolExecutionRecord):
        ...
        recent_records = [r for r in list(self.execution_records)[-1000:] 
        ...
        for i in range(24):
            ...
            hour_end = now - i * 3600
            hour_count = sum(1 for r in recent_records 
                           if hour_start <= r.start_time < hour_end)
    ...
    def get_tool_performance_report(self, tool_name: str) -> Optional[Dict[str, Any]]:
        ...
        if tool_name in self.performance_baselines:
            ...
            baseline_comparison = {
                "duration_change": ((metrics.avg_duration - baseline.get('avg_duration', 0)) / 
                                  max(baseline.get('avg_duration', 1), 0.001)) * 100,
                "success_rate_change": metrics.success_rate - baseline.get('success_rate', 0),
                "efficiency_change": ((metrics.efficiency_score - baseline.get('efficiency_score', 0)) /
                                    max(baseline.get('efficiency_score', 1), 0.001)) * 100
            }
    ...
    def _generate_recommendations(self, metrics: ToolPerformanceMetrics) -> List[str]:
        ...
        most_common_error = max(metrics.error_patterns.items(), 
    ...
    def get_overall_analytics(self) -> Dict[str, Any]:
        ...
        top_tools = sorted(self.tool_metrics.items(), 
    ...
    def _calculate_hourly_performance(self, records: List[ToolExecutionRecord]) -> Dict[str, List[float]]:
    ...
    def _assess_system_health(self) -> Dict[str, Any]:
        ...
        concerns = []
        failing_tools = [name for name, metrics in self.tool_metrics.items() 
                        if metrics.success_rate < 0.8]
        if failing_tools:
            concerns.append(f"Tools with low success rates: {', '.join(failing_tools)}")
            
        slow_tools = [name for name, metrics in self.tool_metrics.items() 
                     if metrics.p95_duration > 30.0]  # 30 seconds
        if slow_tools:
    ...
    def export_analytics_data(self, filepath: str):
        ...
        data = {
            ...
            "execution_records": [asdict(record) for record in list(self.execution_records)[-1000:]]  # Last 1000
        }
