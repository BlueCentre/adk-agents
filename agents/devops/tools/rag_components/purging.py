...
try:
    ...
    logging.error(f"Failed to import CHROMA_DATA_PATH from .indexing: {e}. Purging tool may not work correctly.")
...
def purge_rag_index_data():
    ...
    logging.warning(f"This will affect the collection: {CHROMA_COLLECTION_NAME if CHROMA_COLLECTION_NAME else 'Unknown'}")
...
if __name__ == "__main__":
    ...
    if not CHROMA_DATA_PATH:
        ...
        print(f"This is expected to contain data for collection: {CHROMA_COLLECTION_NAME if CHROMA_COLLECTION_NAME else 'Unknown'}")
        
        confirmation = input("Are you absolutely sure you want to continue? (yes/no): ").strip().lower()
        if confirmation == 'yes':
