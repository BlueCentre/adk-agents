...
def create_chroma_client():
    ...
    try:
        ...
        test_collection = client.get_or_create_collection(name="test_write_access")
        return client
    except Exception as e:
...
def index_file_chunks(collection, file_chunks_data: list[dict]):
    ...
    try:
        ...
        if "readonly database" in str(e).lower():
            logger.warning("Detected readonly database issue. Attempting to recreate the client in a writable location...")
            global chroma_client, CHROMA_DATA_PATH
            
            # Try to force recreation of the client in a new location
            new_path = os.path.join(tempfile.gettempdir(), f"devops_agent_chroma_retry_{os.getpid()}")
            os.makedirs(new_path, exist_ok=True)
            CHROMA_DATA_PATH = new_path
...
def clear_index():
    ...
    try:
        ...
        if "readonly database" in str(e).lower():
            global chroma_client, CHROMA_DATA_PATH
            logger.warning("Detected readonly database issue during clear operation. Recreating client...")
