...
class DevOpsAgentLogger:
    ...
    def setup_logging(self):
        ...
        if os.getenv("DEVOPS_AGENT_QUIET", "").lower() not in ("true", "1", "yes"):
    ...
    def start_operation(self, operation_name: str, **context) -> str:
        ...
        op_context = {"operation": operation_name, **context}
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"Started operation: {operation_name}", extra={"operation_start": True, **context}
        )
    ...
    def end_operation(
        ...
        operation_name: str,
        success: bool = True,
        correlation_id_to_end: Optional[str] = None,
        **context,
    ):
        ...
        logger.info(
            ...
            extra={"operation_end": True, "success": success, **context},
        )
...
def log_security_event(event_type: str, details: dict[str, Any]):
    ...
    logger.warning(
        ...
        extra={"event_type": "security", "security_event_type": event_type, **details},
    )
...
def log_business_event(event_type: str, details: dict[str, Any]):
    ...
    logger.info(
        ...
        extra={"event_type": "business", "business_event_type": event_type, **details},
    )
...
def set_user_context(user_id: str, session_id: Optional[str] = None):
    ...
    logger.info(
        f"User context set: {user_id}", extra={"user_id": user_id, "session_id": session_id}
    )


def log_tool_usage(
    tool_name: str, input_size: int, output_size: int, duration: float, success: bool, **metadata
):
...
def log_context_management(
    ...
    **details,
    ...
    logger.debug(
        f"Context management: {operation}",
        extra={
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                agent_logger.end_operation(
                    operation_name, success=True, correlation_id_to_end=corr_id
                )
                return result
            except Exception as e:
                agent_logger.end_operation(
                    operation_name, success=False, correlation_id_to_end=corr_id, error=str(e)
                )
                raise
