...
GRAFANA_EXPORT_INTERVAL_SECONDS = int(
    os.getenv("GRAFANA_EXPORT_INTERVAL_SECONDS", "120").split("#")[0].strip()
)
...
OPENLIT_COLLECT_GPU_STATS = os.getenv("OPENLIT_COLLECT_GPU_STATS", "false").lower() in (
    "true",
    "1",
    "yes",
)
OPENLIT_DISABLE_METRICS = os.getenv("OPENLIT_DISABLE_METRICS", "false").lower() in (
    "true",
    "1",
    "yes",
)
OPENLIT_CAPTURE_CONTENT = os.getenv("OPENLIT_CAPTURE_CONTENT", "true").lower() in (
    "true",
    "1",
    "yes",
)
OPENLIT_DISABLE_BATCH = os.getenv("OPENLIT_DISABLE_BATCH", "false").lower() in (
    "true",
    "1",
    "yes",
)
...
SOFTWARE_ENGINEER_CONTEXT = os.getenv(
    "SOFTWARE_ENGINEER_CONTEXT", "eval/project_context_empty.json"
)

# --- Agent Control Configuration ---
DEVOPS_AGENT_INTERACTIVE = os.getenv("DEVOPS_AGENT_INTERACTIVE", "").lower() in (
    "true",
    "1",
    "yes",
)
...
GEMINI_PRO_TOKEN_LIMIT_FALLBACK = (
    1048576  # Standard Pro is 1M, can be up to 2M for some versions/APIs
)
