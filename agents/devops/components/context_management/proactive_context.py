...
class ProactiveContextGatherer:
    ...
    def _gather_project_files(self) -> list[dict[str, Any]]:
        ...
        important_files = [
            ...
            "setup.py",
            "setup.cfg",
            "uv.lock",  # uv.lock for uv projects
            "Dockerfile",
            "docker-compose.yml",
            "docker-compose.yaml",
            ".env.example",
    ...
    def _read_file_safely(self, file_path: Path) -> Optional[str]:
        ...
        try:
            ...
            if file_path.stat().st_size > self.max_file_size:
                logger.debug(
                    f"Skipping {file_path.name} - too large ({file_path.stat().st_size} bytes)"
                )
                return None
    ...
    def _get_file_type(self, filename: str) -> str:
        ...
        if filename_lower.startswith("readme"):
            return "readme"
        if filename_lower in ["package.json", "package-lock.json"]:
            return "nodejs"
        if filename_lower in ["requirements.txt", "requirements-dev.txt"]:
            return "python_legacy"  # Legacy pip requirements
        if filename_lower in ["pyproject.toml", "setup.py", "setup.cfg"]:
            return "python_modern"  # Modern Python packaging (uv compatible)
        if filename_lower in ["uv.lock"]:
            return "python_uv_lock"  # uv lockfile
        if filename_lower in ["dockerfile", "docker-compose.yml", "docker-compose.yaml"]:
            return "docker"
        if filename_lower in ["makefile"]:
            return "build"
        if filename_lower.startswith(("changelog", "history")):
            return "changelog"
        if filename_lower in ["license", "license.txt", "license.md"]:
            return "license"
        if filename_lower.startswith("contributing"):
            return "contributing"
        if filename_lower.endswith("ignore"):
            return "ignore_file"
        return "config"
    ...
    def _is_documentation_file(self, file_path: Path) -> bool:
        ...
        return bool(any(keyword in name_lower for keyword in doc_keywords))
