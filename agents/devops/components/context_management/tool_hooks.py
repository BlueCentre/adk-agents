...
def _add_snippet_to_state(
    ...
    for snippet in code_snippets:
        if (
            snippet.get("file_path") == file_path
            and snippet.get("start_line") == start_line
            and snippet.get("end_line") == end_line
        ):
...
def process_read_file_results(
    ...
    logger.info(
        f"DEBUG: Tool object type: {type(tool)}, tool.name: {getattr(tool, 'name', 'NO_NAME')}"
    )
    ...
    if hasattr(result, "content"):
        ...
        if isinstance(content_obj, list) and len(content_obj) > 0:
            ...
            content = content_item.text if hasattr(content_item, "text") else str(content_item)
        else:
        ...
        if isinstance(content_obj, list) and len(content_obj) > 0:
            content = (
                content_obj[0].text if hasattr(content_obj[0], "text") else str(content_obj[0])
            )
        else:
            content = str(content_obj)
        ...
        if hasattr(result, "__str__"):
            content = str(result)
            logger.info("DEBUG: Fallback to string representation")
        else:
            return
    ...
    if hasattr(tool, "args") and tool.args:
        ...
        if isinstance(tool.args, dict):
            file_path = (
                tool.args.get("path") or tool.args.get("filepath") or tool.args.get("target_file")
            )
        else:
            # Try accessing as attributes
            file_path = (
                getattr(tool.args, "path", None)
                or getattr(tool.args, "filepath", None)
                or getattr(tool.args, "target_file", None)
            )
    ...
    try:
        ...
        if end_line >= len(lines) - 5:  # Full or near-full file read
            ...
            if len(lines) > 100:
                ...
                for i in range(0, len(lines), chunk_size - overlap):
                    ...
                    _add_snippet_to_state(
                        code_snippets, file_path, chunk_content, i + 1, end_idx, 1.2
                    )
            else:
        ...
        logger.info(
            f"Enhanced code snippet capture from read_file: {file_path} ({len(lines)} lines)"
        )
...
def process_edit_file_results(
    ...
    logger.info(
        f"DEBUG EDIT: Tool object type: {type(tool)}, tool.name: {getattr(tool, 'name', 'NO_NAME')}"
    )
    if hasattr(tool, "args"):
    ...
    if not file_path:
        logger.warning("Missing file path for process_edit_file_results.")
        return
    ...
    try:
        ...
        file_change_tracker.register_file_edit(file_path, new_content)
        ...
        state["app:code_snippets"] = code_snippets
        logger.info(
            f"Enhanced code snippet capture from edit_file: {file_path} ({len(lines)} lines)"
        )
        ...
        for snippet in code_snippets:
            if (
                ...
                snippet["relevance_score"] = (
                    snippet.get("relevance_score", 1.0) + 0.1
                )  # Slightly lower relevance boost than direct read/edit
                found = True
                break
...
def process_execute_shell_command_results(
    ...
    if "stdout" in result or "stderr" in result:
        ...
        command = result.get(
            "command_executed", result.get("command", "Unknown command")
        )  # Also check for 'command' key
