...
class CrossTurnCorrelator:
    ...
    def correlate_context_items(
        ...
        cross_correlations = self._build_cross_correlations(code_snippets, tool_results, conversation_turns)
        
        # Add correlation scores to items
        enhanced_snippets = self._enhance_with_correlations(code_snippets, snippet_correlations, 'snippet')
        ...
        for i, snippet1 in enumerate(snippets):
            ...
            for j, snippet2 in enumerate(snippets):
                ...
                score.temporal_proximity = self._calculate_temporal_proximity(
                    snippet1.get('last_accessed', 0), snippet2.get('last_accessed', 0)
                )
                ...
                if score.final_score > self.snippet_correlation_threshold:  # Only store meaningful correlations
    ...
    def _build_tool_correlations(
        ...
        for i, tool1 in enumerate(tool_results):
            ...
            for j, tool2 in enumerate(tool_results):
                ...
                if score.final_score > self.tool_correlation_threshold:  # Only store meaningful correlations
                    correlations[i][j] = score
    ...
    def _build_cross_correlations(
        ...
        for i, snippet in enumerate(code_snippets):
            for j, tool in enumerate(tool_results):
                ...
                if score.final_score > self.cross_correlation_threshold:  # Higher threshold for cross-correlations
    ...
    def _calculate_content_similarity(self, content1: str, content2: str) -> float:
        ...
        code_patterns = [
            ...
            r'from\s+(\w+)', r'function\s+(\w+)', r'const\s+(\w+)'
        ]
    ...
    def _enhance_with_correlations(
        ...
        for i, item in enumerate(items):
            ...
            enhanced_item['_correlations'] = {
                ...
                'max_score': max((score.final_score for score in item_correlations.values()), default=0.0),
    ...
    def _log_correlation_analysis(
        ...
        snippet_correlations = sum(len(s.get('_correlations', {}).get('related_indices', [])) for s in snippets)
        tool_correlations = sum(len(t.get('_correlations', {}).get('related_indices', [])) for t in tools)
        
        cross_snippet_tools = sum(len(s.get('_cross_correlations', {}).get('tools', [])) for s in snippets)
        cross_tool_snippets = sum(len(t.get('_cross_correlations', {}).get('snippets', [])) for t in tools)
        ...
        for i, snippet in enumerate(snippets[:3]):  # Top 3 snippets
            ...
            if correlations.get('count', 0) > 0:
                ...
                logger.info(f"    Snippet {i+1} ({file_path}): {related_count} correlations (max: {max_score:.3f})")
        ...
        for i, tool in enumerate(tools[:3]):  # Top 3 tools
            ...
            if correlations.get('count', 0) > 0:
                ...
                logger.info(f"    Tool {i+1} ({tool_name}): {related_count} correlations (max: {max_score:.3f})") 