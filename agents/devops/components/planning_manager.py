...
class PlanningManager:
    ...
    def _should_trigger_heuristic(self, user_message_content: str) -> bool:
        ...
        for pattern in simple_exploration_patterns:
            if re.search(pattern, lower_user_message):
                logger.info(
                    f"PlanningManager: Simple exploration detected ({pattern}), skipping planning."
                )
                return False
    ...
    def _is_plan_related_feedback(self, user_message: str) -> bool:
        ...
        if len(user_message.split()) <= 8 and any(
            word in lower_message for word in ["what", "how", "where", "when", "who", "why"]
        ):
            return False
    ...
    async def handle_before_model_planning_logic(
        ...
        try:
            ...
            if (
                not self.is_awaiting_plan_approval
                and agent_config.ENABLE_INTERACTIVE_PLANNING
                and user_message_content
            ):
                if self._should_trigger_heuristic(user_message_content):
                    ...
                    try:
                        planning_prompt_text = agent_prompts.PLANNING_PROMPT_TEMPLATE.format(
                            user_request=safe_user_request, code_context_section=safe_code_context
                        )
                        ...
                        planning_prompt_text = (
                            f"Create a comprehensive plan for this request: {safe_user_request}"
                        )

                    try:
                        llm_request.contents = [
                            genai_types.Content(
                                parts=[genai_types.Part(text=planning_prompt_text)], role="user"
                            )
                        ]
                        ...
                        logger.error(
                            f"PlanningManager: Error modifying LLM request: {request_error}"
                        )
            ...
            logger.error(
                f"PlanningManager: Unexpected error in handle_before_model_planning_logic: {e}",
                exc_info=True,
            )
        ...
        try:
            if self.is_plan_generation_turn:
                ...
                if plan_text:
                    ...
                    return LlmResponse(
                        content=genai_types.Content(parts=[response_part]),
            ...
            logger.error(
                f"PlanningManager: Unexpected error in handle_after_model_planning_logic: {e}",
                exc_info=True,
            )
            # Reset state to prevent stuck conditions
            self.reset_planning_state()
