...
class StateManager:
    ...
    def get_state_for_context(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "temp:tool_results_current_turn": self.current_turn.tool_results
            if self.current_turn
            else [],
...
class MyDevopsAgent(LlmAgent):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        logger.info(
            f"Agent {self.name} initialized with token limit: {self._actual_llm_token_limit}"
        )
    ...
    async def handle_before_model(
                ...
                if user_message_content:
                    if not self._state_manager.current_turn:
                        ...
                        if self._state_manager.current_turn.user_message:
                            ...
                            combined_message = (
                                self._state_manager.current_turn.user_message
                                + "\n"
        ...
        (
            planning_response,
            approved_plan_text,
        ) = await self._planning_manager.handle_before_model_planning_logic(
...
Begin execution now, starting with the first step."""
            ...
            if hasattr(llm_request, "contents") and isinstance(
                ...
                execution_content = genai_types.Content(
                    role="user", parts=[genai_types.Part(text=execution_instruction)]
                )
                ...
                for content in llm_request.contents:
                    if content.role == "user" and not user_message_replaced:
                        ...
                        logger.info(
                            "MyDevopsAgent: Replaced user message with plan execution instruction."
                        )
        ...
        if not is_currently_a_plan_generation_turn:
            ...
            logger.info(
                "🔧 OPTIMIZATION: Excluding conversation history from base prompt calculation"
            )
            logger.info(
                "   Conversation history will be managed by context manager with smart optimization"
            )
            ...
            if hasattr(llm_request, "contents") and isinstance(
                ...
                if user_message_content:
                    ...
                    current_user_in_contents = any(
                        ...
                        and content.parts
                        and any(
                            hasattr(part, "text") and part.text == user_message_content
                            for part in content.parts
                ...
                if hasattr(llm_request, "model") and llm_request.model:
                    ...
                    if hasattr(llm_request, "contents") and isinstance(
                        ...
                        try:
                            ...
                            self._log_final_prompt_analysis(
                                llm_request, context_dict, system_context_message
                            )
                    ...
                    logger.warning(
                        "Skipping structured context injection as LlmRequest seems incomplete."
                    )
            except Exception as e:
                logger.error(
                    f"Failed to inject structured context: {e} - LlmRequest: {llm_request}",
                    exc_info=True,
                )
        else:
            logger.info(
    ...
    async def handle_after_model(
        ...
        planning_intercept_response = (
            await self._planning_manager.handle_after_model_planning_logic(
        ...
        if hasattr(llm_response, "usage_metadata") and llm_response.usage_metadata:
            ...
            if thinking_tokens > 0:
                ui_utils.display_model_usage_with_thinking(
                    self._console,
        ...
        try:
            if (
                self._state_manager.current_turn
                and self._state_manager.current_turn.phase != TurnPhase.COMPLETED
            ):
    ...
    async def handle_before_tool(
        ...
        with trace_tool_execution(tool.name, operation="preprocessing", args=args):
            try:
                ...
                logger.error(
                    f"Agent {self.name}: Error in handle_before_tool: {e}",
                    exc_info=True,
                )
                return None
    ...
    async def handle_after_tool(
        ...
        if not (
            mcp_types and isinstance(tool_response, mcp_types.CallToolResult)
        ) and not isinstance(tool_response, ExecuteVettedShellCommandOutput):
            if isinstance(tool_response, dict) and (
                tool_response.get("status") == "error" or tool_response.get("error")
            ):
                ...
                if tool_response.get("retry_suggested"):
                    ui_utils.display_tool_error_with_suggestions(
    ...
    def _extract_response_text(self, llm_response: LlmResponse) -> Optional[str]:
        try:
            ...
            logger.warning(
                f"Failed to extract text from LLM response: {e}. "
    ...
    def _process_llm_response(self, llm_response: LlmResponse) -> Dict[str, Any]:
                ...
                for i, part in enumerate(content_obj.parts):
                    if hasattr(part, "text") and part.text is not None:
                        ...
                        if hasattr(part, "thought") and part.thought:
                            ...
                            logger.debug(
                                f"Part {i}: Extracted as thought summary (length: {len(part.text)})"
                            )
                        else:
        ...
        if (
            direct_text
            and not extracted_data["text_parts"]
            and not extracted_data["function_calls"]
        ):
        ...
        if extracted_data["function_calls"]:
            logger.info(
                f"Detected function calls in LLM response: {extracted_data['function_calls']}"
            )
    ...
    def _create_filtered_response(
        ...
        try:
            ...
            for func_call in processed_response["function_calls"]:
                ...
                if (
                    hasattr(original_response, "content")
                    and original_response.content
                    and original_response.content.parts
                ):
    ...
    def _assemble_context_from_state(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self._context_manager.current_turn_number = len(
            self._context_manager.conversation_turns
        )  # Update current turn number
        logger.info(
            f"Synchronized {len(self._context_manager.conversation_turns)} conversation turns"
        )
        ...
        for snippet_data in code_snippets_data:
            snippet = CodeSnippet(
                ...
                last_accessed=snippet_data.get(
        ...
        self._context_manager.state.core_goal = state.get(
            "app:core_goal", self._context_manager.state.core_goal
        )
        self._context_manager.state.current_phase = state.get(
            "app:current_phase", self._context_manager.state.current_phase
        )
        self._context_manager.state.key_decisions = state.get(
            "app:key_decisions", self._context_manager.state.key_decisions
        )
        self._context_manager.state.last_modified_files = state.get(
            "app:last_modified_files", self._context_manager.state.last_modified_files
        )
    ...
    def _log_final_prompt_analysis(
        ...
        logger.info(
            f"Token Utilization: {(total_prompt_tokens / self._actual_llm_token_limit * 100):.1f}%"
        )
        logger.info(
            f"Remaining Capacity: {self._actual_llm_token_limit - total_prompt_tokens:,} tokens"
        )
        logger.info("")
        ...
        while i < len(contents):
            ...
            if content.role == "system":
                ...
                if (
                    ...
                    and content.parts[0].text.startswith("SYSTEM CONTEXT (JSON):")
                ):
        ...
        for conversation in reversed(
            ...
            if isinstance(conversation, list):
                ...
                if (
                    ...
                    logger.info(
                        f"🔧 FILTERED: Older conversation segment with {len(conversation)} messages"
                    )
            else:
    ...
    def _is_retryable_error(self, error_message: str, error_type: str) -> bool:
        ...
        if any(
            ...
            for pattern in ["429", "RESOURCE_EXHAUSTED", "quota", "rate limit"]
        ):
            return True
        ...
        if any(
            pattern in error_message_lower
            for pattern in ["token", "context length", "too long", "maximum context"]
        ):
            return True
    ...
    async def _optimize_input_for_retry(
        ...
        try:
            ...
            if retry_attempt == 1:
                ...
                logger.info(
                    "Agent optimization level 1: Reducing conversation history and code snippets"
                )
                ...
                if (
                    self._state_manager.current_turn
                    and self._state_manager.current_turn.tool_results
                ):
            ...
            logger.error(
                f"Agent {self.name}: Error during input optimization: {e}",
                exc_info=True,
            )
            return False
    ...
    async def _run_async_impl(
        ...
        try:
            while retry_count <= max_retries:
                ...
                try:
                    logger.info(
                        f"Agent {self.name}: Starting attempt {retry_count + 1}/{max_retries + 1}"
                    )
                    ...
                    if should_retry and retry_count < max_retries:
                        ...
                        optimization_success = await self._optimize_input_for_retry(
                            ctx, retry_count
                        )
                        ...
                        logger.info(
                            f"Agent {self.name}: Waiting {delay:.1f} seconds before retry..."
                        )
            ...
            if ("429" in error_message and "RESOURCE_EXHAUSTED" in error_message) or (
                ...
                ui_utils.display_unhandled_error(
                    self._console, error_type, error_message, mcp_related_hint
                )
