...
class SmartPrioritizationValidator:
    ...
    def test_content_relevance(self):
        ...
        if auth_files and non_auth_files:
            ...
            print(
                "✅ PASS: Auth files ranked higher"
                if success
                else "❌ FAIL: Auth files not prioritized"
            )
    ...
    def test_recency_impact(self):
        ...
        print(
            "✅ PASS: Recent access boosted ranking"
            if success
            else "❌ FAIL: Recency not properly weighted"
        )
    ...
    def test_error_priority(self):
        ...
        print(
            "✅ PASS: Error content prioritized"
            if success
            else "❌ FAIL: Error priority not working"
        )
    ...
    def test_context_coherence(self):
        ...
        relevant_files = [
            s for s in ranked_snippets if any(keyword in s["file"] for keyword in ["api", "model"])
        ]
        irrelevant_files = [
            s
            for s in ranked_snippets
            if not any(keyword in s["file"] for keyword in ["api", "model"])
        ]
        ...
        if relevant_files and irrelevant_files:
            ...
            print(
                "✅ PASS: Contextually relevant files prioritized"
                if success
                else "❌ FAIL: Context coherence not working"
            )
    ...
    def test_frequency_score(self):
        ...
        print(
            "✅ PASS: Frequency scoring working"
            if success
            else "❌ FAIL: Frequency not properly weighted"
        )
    ...
    def test_end_to_end_ranking(self):
        ...
        current_context = (
            "fixing authentication bug in login system with database connection issues"
        )
        current_turn = 25
        ...
        print(
            "\n✅ PASS: Ranking follows expected priority"
            if success
            else "❌ FAIL: Unexpected ranking order"
        )
    ...
    def test_tool_results_prioritization(self):
        ...
        print(
            "✅ PASS: Error result prioritized" if success else "❌ FAIL: Error not ranked highest"
        )
