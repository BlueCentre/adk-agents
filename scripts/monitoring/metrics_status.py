...
def check_grafana_cloud_status():
    ...
    export_disabled = os.getenv("DEVOPS_AGENT_DISABLE_TELEMETRY_EXPORT", "").lower() in (
        "true",
        "1",
        "yes",
    )
...
def show_recommendations():
    ...
    if export_interval < 300:
        recommendations.append(
            "Consider increasing GRAFANA_EXPORT_INTERVAL_SECONDS to 300+ for rate limiting"
        )
    ...
    if not gpu_stats:
        recommendations.append(
            "Enable GPU monitoring with OPENLIT_COLLECT_GPU_STATS=true if you have GPU"
        )
    ...
    if environment == "Production":
        recommendations.append(
            "Consider setting OPENLIT_ENVIRONMENT=Development for local development"
        )
...
def main():
    ...
    print(
        "  • Local dashboard: uvx --with rich --with psutil python scripts/telemetry_dashboard.py"
    )
