...
class TestLoadFromSubmodule:
  ...
  def test_load_from_submodule_dependency_error(self):
    ...
    with patch(
      ...
      with pytest.raises(
          ValueError, match="Fail to load 'test_agent.agent' module"
      ):
  ...
  def test_perform_load_with_agents_dir_success_from_module(self):
    ...
    with patch('src.wrapper.adk.cli.utils.agent_loader.sys.path') as mock_path:
      ...
      with patch(
        ...
        with patch.object(
            loader, '_load_from_module_or_package', return_value=mock_agent
        ):
  ...
  def test_perform_load_with_agents_dir_success_from_submodule(self):
    ...
    with patch('src.wrapper.adk.cli.utils.agent_loader.sys.path') as mock_path:
      ...
      with patch(
        ...
        with patch.object(
            loader, '_load_from_module_or_package', return_value=None
        ):
          with patch.object(
              loader, '_load_from_submodule', return_value=mock_agent
          ):
            result = loader._perform_load('test_agent')
...
class TestLoadAgent:
  ...
  def test_load_agent_not_in_cache(self):
    ...
    with patch.object(
        loader, '_perform_load', return_value=mock_agent
    ) as mock_perform:
      result = loader.load_agent('test_agent')
...
class TestLoadAgentFromModule:
  ...
  def test_load_agent_from_module_wrong_type(self):
    ...
    with patch(
      ...
      with pytest.raises(
          ValueError,
          match=(
              'Failed to load agent from module.*Error:.*is not an instance of'
              ' BaseAgent'
          ),
      ):
        load_agent_from_module('my.agent.module')
  ...
  def test_load_agent_from_module_not_found(self):
    ...
    with patch(
      ...
      mock_import.side_effect = ModuleNotFoundError(
          "No module named 'my.agent.module'"
      )
