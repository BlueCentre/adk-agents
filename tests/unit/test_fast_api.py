...
class TestAgentChangeEventHandler:
    ...
    def test_on_modified_python_file(self):
        ...
        with (
            patch("src.wrapper.adk.cli.fast_api._app_name", "test_app"),
            patch("src.wrapper.adk.cli.fast_api._runners_to_clean", set()) as mock_runners_to_clean,
            patch("src.wrapper.adk.cli.fast_api.logger") as mock_logger,
        ):
    ...
    def test_on_modified_ignored_file_types(self):
        ...
        for file_path in ignored_files:
            ...
            with (
                patch("src.wrapper.adk.cli.fast_api._app_name", "test_app"),
                patch(
                    "src.wrapper.adk.cli.fast_api._runners_to_clean", set()
                ) as mock_runners_to_clean,
                patch("src.wrapper.adk.cli.fast_api.logger") as mock_logger,
            ):
                handler.on_modified(event)
...
class TestFastAPIApp:
    ...
    def mock_services(self):
        ...
        with (
            patch("src.wrapper.adk.cli.fast_api.InMemorySessionService") as mock_session,
            patch("src.wrapper.adk.cli.fast_api.InMemoryMemoryService") as mock_memory,
            patch("src.wrapper.adk.cli.fast_api.InMemoryArtifactService") as mock_artifact,
            patch("src.wrapper.adk.cli.fast_api.InMemoryCredentialService") as mock_credential,
            patch("src.wrapper.adk.cli.fast_api.LocalEvalSetsManager") as mock_eval_sets,
            patch("src.wrapper.adk.cli.fast_api.LocalEvalSetResultsManager") as mock_eval_results,
    ...
    def test_list_apps_invalid_path(self):
        ...
        app = get_fast_api_app(agents_dir="non_existent_path", web=False, trace_to_cloud=False)
    ...
    async def test_get_session_success(self, test_client, mock_services):
        ...
        response = test_client.get("/apps/test_app/users/test_user/sessions/test_session")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_get_session_not_found(self, test_client, mock_services):
        ...
        response = test_client.get("/apps/test_app/users/test_user/sessions/non_existent")
        assert response.status_code == 404
    ...
    @pytest.mark.asyncio
    async def test_create_session_with_id_success(self, test_client, mock_services):
        ...
        mock_services["session"].get_session.return_value = None  # Session doesn't exist
    ...
    @pytest.mark.asyncio
    async def test_create_session_with_id_already_exists(self, test_client, mock_services):
        ...
        response = test_client.post("/apps/test_app/users/test_user/sessions/existing_session")
    ...
    def test_create_eval_set_validation_error(self, test_client, mock_services):
        ...
        mock_services["eval_sets"].create_eval_set.side_effect = ValueError("Invalid eval set")
    ...
    async def test_delete_session(self, test_client, mock_services):
        ...
        response = test_client.delete("/apps/test_app/users/test_user/sessions/test_session")
        assert response.status_code == 200
    ...
    async def test_list_artifact_names(self, test_client, mock_services):
        ...
        response = test_client.get("/apps/test_app/users/test_user/sessions/test_session/artifacts")
        assert response.status_code == 200
        data = response.json()
...
class TestServiceConfiguration:
    ...
    def test_memory_service_rag_configuration(self):
        ...
        with (
            ...
            patch("src.wrapper.adk.cli.fast_api.VertexAiRagMemoryService") as mock_rag,
        ):
    ...
    def test_session_service_agentengine_configuration(self):
        ...
        with (
            ...
            patch("src.wrapper.adk.cli.fast_api.VertexAiSessionService") as mock_session,
        ):
    ...
    def test_session_service_database_configuration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("src.wrapper.adk.cli.fast_api.DatabaseSessionService") as mock_db:
    ...
    def test_cloud_tracing_configuration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch.dict(os.environ, {"GOOGLE_CLOUD_PROJECT": "test-project"}):
                with patch("src.wrapper.adk.cli.fast_api.CloudTraceSpanExporter") as mock_exporter:
                    mock_exporter_instance = Mock()
                    mock_exporter.return_value = mock_exporter_instance

                    get_fast_api_app(agents_dir=temp_dir, trace_to_cloud=True, web=False)
...
class TestAdditionalEndpoints:
    ...
    def test_get_eval_case_success(self, test_client, mock_services):
        ...
        eval_case = EvalCase(
            eval_id="test_eval",
            conversation=[],
            session_input=SessionInput(app_name="test_app", user_id="test_user", state={}),
            creation_timestamp=time.time(),
        )
        mock_services["eval_sets"].get_eval_case.return_value = eval_case

        response = test_client.get("/apps/test_app/eval_sets/test_set/evals/test_eval")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_eval_case_not_found(self, test_client, mock_services):
        ...
        response = test_client.get("/apps/test_app/eval_sets/test_set/evals/missing_eval")
        assert response.status_code == 404
    ...
    def test_update_eval_not_found(self, test_client, mock_services):
        ...
        mock_services["eval_sets"].update_eval_case.side_effect = NotFoundError("Eval not found")
    ...
    def test_delete_eval_success(self, test_client, mock_services):
        ...
        response = test_client.delete("/apps/test_app/eval_sets/test_set/evals/test_eval")
        assert response.status_code == 200
    ...
    def test_delete_eval_not_found(self, test_client, mock_services):
        ...
        mock_services["eval_sets"].delete_eval_case.side_effect = NotFoundError("Eval not found")

        response = test_client.delete("/apps/test_app/eval_sets/test_set/evals/missing_eval")
        assert response.status_code == 404
    ...
    @pytest.mark.asyncio
    async def test_add_session_to_eval_set_success(self, test_client, mock_services):
        ...
        with (
            ...
            patch("src.wrapper.adk.cli.fast_api.create_empty_state") as mock_create_state,
        ):
    ...
    @pytest.mark.asyncio
    async def test_add_session_to_eval_set_validation_error(self, test_client, mock_services):
        ...
        mock_services["session"].get_session.return_value = session
        mock_services["eval_sets"].add_eval_case.side_effect = ValueError("Invalid eval case")
        mock_services["agent_loader"].load_agent.return_value = Mock()
    ...
    @pytest.mark.asyncio
    async def test_load_artifact_version_not_found(self, test_client, mock_services):
    ...
    def test_get_eval_result_validation_error(self, test_client, mock_services):
        ...
        mock_services[
            "eval_results"
        ].get_eval_set_result.side_effect = ValidationError.from_exception_data(
            "ValidationError",
...
""")
    ...
    def test_eval_storage_gcs_configuration(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("src.wrapper.adk.cli.fast_api.LocalEvalSetsManager") as mock_local:
...
class TestAdvancedEndpoints:
    ...
    async def test_run_eval_success(self, test_client, mock_services):
        ...
        with patch("google.adk.cli.cli_eval.run_evals", mock_run_evals):
            ...
            assert "evalId" in results[0] or "eval_id" in results[0] or len(results[0]) > 0
    ...
    @pytest.mark.asyncio
    async def test_run_eval_module_not_found_error(self, test_client, mock_services):
    ...
    @pytest.mark.asyncio
    async def test_agent_run_sse_session_not_found(self, test_client, mock_services):
    ...
    @pytest.mark.asyncio
    async def test_get_event_graph_function_responses(self, test_client, mock_services):
    ...
    @pytest.mark.asyncio
    async def test_get_event_graph_no_function_calls_or_responses(self, test_client, mock_services):
    ...
    def test_get_fast_api_app_reload_agents_enabled_observer_setup(self, temp_agents_dir):
        ...
        with patch("src.wrapper.adk.cli.fast_api.Observer") as mock_observer_class:
            ...
            with patch("src.wrapper.adk.cli.fast_api.AgentLoader") as mock_agent_loader_class:
                ...
                get_fast_api_app(
                    agents_dir=temp_agents_dir, reload_agents=True, web=False, trace_to_cloud=False
                )
    ...
    def test_get_fast_api_app_reload_agents_disabled_no_observer(self, temp_agents_dir):
        ...
        with patch("src.wrapper.adk.cli.fast_api.Observer") as mock_observer_class:
            ...
            with patch("src.wrapper.adk.cli.fast_api.AgentLoader") as mock_agent_loader_class:
                ...
                get_fast_api_app(
                    agents_dir=temp_agents_dir, reload_agents=False, web=False, trace_to_cloud=False
                )
    ...
    def test_get_fast_api_app_reload_agents_lifespan_cleanup(self, temp_agents_dir):
        ...
        with patch("src.wrapper.adk.cli.fast_api.Observer") as mock_observer_class:
            ...
            with patch("src.wrapper.adk.cli.fast_api.AgentLoader") as mock_agent_loader_class:
                ...
                with patch(
                    "src.wrapper.adk.cli.fast_api.cleanup.close_runners", new_callable=AsyncMock
                ):
                    get_fast_api_app(
                        agents_dir=temp_agents_dir,
                        reload_agents=True,
