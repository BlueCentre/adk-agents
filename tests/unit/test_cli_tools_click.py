...
warnings.filterwarnings("ignore", ".*Remove its duplicate as parameters should be unique.*")
...
class TestValidateExclusive:
    ...
    def test_validate_exclusive_second_option_raises_error(self):
        ...
        assert "Options 'option2' and 'option1' cannot be set together" in str(exc_info.value)
...
class TestCLICommands:
    ...
    def test_cli_run_with_options(self, mock_log_to_tmp, mock_run_cli, mock_asyncio_run):
        ...
        try:
            result = self.runner.invoke(cli_run, [
                '--save_session',
    ...
    def test_cli_web_with_options(self, mock_setup_logger, mock_get_app, mock_server_class):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = self.runner.invoke(cli_web, [
                '--host', '0.0.0.0',
                '--port', '9000',
            ...
            assert os.path.realpath(call_kwargs['agents_dir']) == os.path.realpath(temp_dir)
    ...
    def test_cli_web_packaged_basic(self, mock_dirname, mock_exists, mock_setup_logger, mock_get_app, mock_server_class):
        ...
        with patch('src.wrapper.adk.cli.cli_tools_click.agents', create=True) as mock_agents:
    ...
    def test_cli_deploy_cloud_run_basic(self, mock_deploy):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = self.runner.invoke(cli_deploy_cloud_run, [
                '--project', 'test-project',
                '--region', 'us-central1',
                temp_dir
            ])
            ...
            # Use os.path.realpath to handle platform path differences
            assert os.path.realpath(call_kwargs['agent_folder']) == os.path.realpath(temp_dir)
    ...
    def test_cli_deploy_agent_engine_basic(self, mock_deploy):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = self.runner.invoke(cli_deploy_agent_engine, [
                '--project', 'test-project',
                '--region', 'us-central1', 
...
class TestErrorHandling:
    ...
    def test_web_packaged_agents_not_found(self):
        ...
        with patch('src.wrapper.adk.cli.cli_tools_click.os.path.exists', return_value=False):
            result = self.runner.invoke(cli_web_packaged, [])
