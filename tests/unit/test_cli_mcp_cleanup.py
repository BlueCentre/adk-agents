...
class TestMCPCleanupExceptionHandling(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.test_errors = [
            ...
            (RuntimeError("Attempted to exit cancel scope in a different task than it was entered in"), True),
        ...
        async def test_mcp_error_handling():
            try:
                ...
                if is_mcp_cleanup_error:
                    mock_logger.warning(f"MCP cleanup completed with expected async context warnings: {error_msg}")
                    return "handled"
                else:
                    raise
