...
class TestRunCli:
  ...
  async def test_run_cli_with_saved_session(
    ...
    try:
      # Test function
      await run_cli(
          agent_module_name='test_agent', saved_session_file=session_file_path
      )
  ...
  async def test_run_cli_with_session_saving(
    ...
    mock_open.assert_called_once_with(
        'test_session_123.session.json', 'w', encoding='utf-8'
    )
    mock_file.write.assert_called_once_with('{"test": "session"}')
    mock_print.assert_called_once_with(
        'Session saved to', 'test_session_123.session.json'
    )
