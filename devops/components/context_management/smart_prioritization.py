...
class SmartPrioritizer:
    ...
    def __init__(self):
        ...
        self.error_keywords = [
            ...
            'stderr', 'traceback', 'stacktrace', 'warning', 'critical'
        ]
        
        self.high_value_keywords = [
            'config', 'setup', 'install', 'deploy', 'build', 'test', 'main',
            'init', 'start', 'run', 'execute', 'import', 'class', 'function',
            'api', 'endpoint', 'service', 'database', 'auth', 'security'
        ]
    ...
    def prioritize_code_snippets(
        ...
        for snippet in snippets:
            ...
            logger.debug(f"  Snippet {snippet.get('file', 'unknown')}:{snippet.get('start_line', 0)}")
            logger.debug(f"    Content: {score.content_relevance:.3f}, Recency: {score.recency_score:.3f}")
            logger.debug(f"    Frequency: {score.frequency_score:.3f}, Error: {score.error_priority:.3f}")
            logger.debug(f"    Coherence: {score.context_coherence:.3f}, Final: {score.final_score:.3f}")
    ...
    def prioritize_tool_results(
        ...
        for i, (score, result) in enumerate(scored_results[:5]):
            ...
            logger.info(f"    {i+1}. {tool_name} (turn {turn}){error_indicator} (score: {score:.3f})")
    ...
    def _calculate_tool_result_relevance(
        ...
        # 3. Error Priority - much higher for error results
        score.error_priority = 1.0 if is_error else self._calculate_error_priority(summary)
        
        # 4. Tool-specific bonuses
        if tool_name in ['read_file', 'edit_file', 'read_file_content', 'edit_file_content']:
    ...
    def _calculate_content_relevance(self, content: str, context: str) -> float:
        ...
        # 2. High-value keyword bonus
        high_value_matches = sum(1 for keyword in self.high_value_keywords 
                               if keyword in content_lower)
        high_value_score = min(high_value_matches * 0.2, 1.0)
        
        # 3. File path relevance (if applicable)
        file_relevance_score = 0.0
        if any(ext in content_lower for ext in ['.py', '.js', '.ts', '.java', '.go', '.rs']):
            file_relevance_score = 0.3  # Source code files are generally relevant
        elif any(ext in content_lower for ext in ['.md', '.txt', '.rst']):
            file_relevance_score = 0.2  # Documentation files moderately relevant
        elif any(ext in content_lower for ext in ['.json', '.yaml', '.yml', '.toml']):
            file_relevance_score = 0.4  # Config files highly relevant
        
        # Combine scores with weights
        final_score = (
            word_overlap_score * 0.5 +
            high_value_score * 0.3 +
            file_relevance_score * 0.2
        )
    ...
    def _calculate_error_priority(self, content: str) -> float:
        ...
        content_lower = content.lower()
        error_matches = sum(1 for keyword in self.error_keywords 
                          if keyword in content_lower)
